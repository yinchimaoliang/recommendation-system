...
class Main():
    ...
    def getData(self):
        ...
        for i in range(self.user_num):
            for j in range(self.users[i].item_num):
                self.rating_matrix[self.user_dic[self.users[i].id], self.item_dic[self.users[i].items[j][0]]] = \
                self.users[i].items[j][1]
    ...
    def computeUserSim(self, user_id, type):
